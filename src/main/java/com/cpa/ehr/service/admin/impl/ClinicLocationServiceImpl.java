...
public class ClinicLocationServiceImpl implements ClinicLocationService {
	...
	public List<ClinicLocationDTO> retrieveAllClinicLocations(Long orgId) {
		try {
			...
			String exceptionString = "Error while retrieving selected clinic location By location Id: " + "{"
	 ...
	 public ClinicLocationDTO persistClinicLocation(ClinicLocationDTO clinicLocationDTO) {
		try {
			if ( clinicLocationDTO != null ) {
			 ...
			 String username = SecurityUtils.getCurrentUserLogin();
				String exceptionString = "Error while inserting clinic location {}  " + "{" + clinicLocationDTO + "} \n"
				+ emailService.getStackTrace(e);
		emailService.sendExceptionEmail(exceptionString,username);
	...
	public ClinicLocationDTO retrieveClinicLocationById(Long clinicLocId){
		try {
			...
			String username = SecurityUtils.getCurrentUserLogin();
			String exceptionString = "Error while removing clinic location {}  " + "{" + clinicLocId + "} \n"
					+ emailService.getStackTrace(e);
			emailService.sendExceptionEmail(exceptionString,username);
	...
	public ClinicLocationDTO updateClinicLocationById(ClinicLocationDTO clinicLocDTOToUpdate){
		try {
			...
			String username = SecurityUtils.getCurrentUserLogin();
			String exceptionString = "Error while updating clinic location {}  " + "{" + clinicLocDTOToUpdate + "} \n"
					+ emailService.getStackTrace(e);
			emailService.sendExceptionEmail(exceptionString,username);
