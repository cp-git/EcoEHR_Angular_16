...
import { EducationalLinks } from "../services/educationLinks";
...
export class HPIComponent implements OnInit {
    ...
    buttonName:any='SAVE';
    allSystemTypes: any[] = [];
    allExternalLinks: EducationalLinks[] = [];
...
//Setting values while update operation
        ...
        .subscribe(data => {
          ...
          //  console.log(this.list)
            for (let i = 0; i < this.list.length; i++) {
                ...
                if(this.list[i].questionType=='calender'){      // Setting Values for Calender
                    // this.selectedTValue[this.list[i].questionId]=option;         
                    this.calenderVal[this.list[i].questionId]=this.list[i].answer;
               ...
               if(this.list[i].questionType=='tdropdown'){     // Setting Values for TDropdown
                ...
                if(this.list[i].optionValue != null){
                    optionValue=this.list[i].optionValue.split("-");
                }
               
                if(this.list[i].answer != null){
                     answer=this.list[i].answer.split(",");
                }
                    
               if((this.list[i].answer !==null) && (this.list[i].optionValue != null)){
    ...
    openTab(sysId, systemDesc, sysCode) {
        this.isCheckedTDD();
        this.modalflag = true;
        if (!this.flag) {
            this.questionGroupId=null;
    ...
    insertData(val){
       // console.log("insert dataaaa called");
        ...
        if (this.QuestionSelectedArray.length > 0) {
            ...
            this.encounterQuestionOptionService.insertQuestionOptions(encounterQuestionOptionList)
                .subscribe(successCode => {
                   ...
                   // console.log("insert dataaaa called statussss codeeee iffff");
                    if(this.data=="edit"){
                       // console.log("insert dataaaa called statussss codeeee iffff editttt");
                 this.router.navigate(['/patients/ros/',this.encounterId,'edit']);
                    }
                    else{
                       // console.log("insert dataaaa called statussss codeeee elseeee");
        ...
        if(this.data=="edit"){
      //  console.log("editttttttttt"+this.encounterId);
        this.router.navigate(['/patients/ros/',this.encounterId,'edit']);
        }
        else{
        //    console.log("insert dataaaa called statussss codeeee elseeeee edittttt");
        this.router.navigate(['/patients/ros/' + this.encounterId]);
        }
    }
    ...
    isFieldValid(event, type) {
        ...
        //logic to check validation for selected calender Date
        if ((type == "calender"))
        {
            event.target.value==undefined || event.target.value=="" ?this.isValidDate[rowNo] = rowNo : this.isValidDate[rowNo] = -1;
            
        }
        else 
        {
            this.isValidDate[rowNo] = -1;
    ...
    calenderdatepicker(v, data) 
    {
        // var self = this;
        $("#calender" + v).datepicker({
        changeMonth: true,
  ...
  openDrp(systemDesc:String){
    this.allExternalLinks = null;
    this.systemServices.getReferenceLinkForSysCode(this.sysCode)
    .subscribe(data => {
        this.systemDetails = data;
        this.url = this.systemDetails.externalLinks;
     
        if(this.systemDetails.externalLinks != null && this.url.length > 0){
            this.allExternalLinks = JSON.parse(this.systemDetails.externalLinks);
        }
    })
